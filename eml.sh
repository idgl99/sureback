...
if ls /home/log/email/*.eml 2>/dev/null 1>&2 
then
  n=0;
  # process each .eml file
  for i in /home/log/email/*.eml
  do
    n=$(expr $n + 1)
    # get rid of CRLFs
    /bin/sed 's/$//' $i > /tmp/a$$.$n
...
#   rm /tmp/a$$.$n /tmp/b$$.$n
    ...
    then
      mv $i $(echo $i | /bin/sed 's/eml$/snt/')
      touch $(echo $i | /bin/sed 's/eml$/snt/')
    fi 
  done
fi
